 ...
 -- Case 3
 -- find golden cross counts over all symbols and save the table as G_Count
 --Query_1 start
create table g_count as
with f_g as
(with grp_s as
(with grp_stock as
(select *,
...
avg(close) over(partition by symbol order by date rows between 199 preceding and current row) as DMA200
 ...
 from grp_s)
 select symbol, count(gc) as gc_count
 from f_g
 where gc=1
 group by symbol
 ...
 -- Finding the percentage of golden cross which resulted in a price increase of 1%,2%,...
 -- Creating a table with all date corresponding to the golden cross and and dates after one week,two week,
...
-- Calculating price increase corresponding to each week and month separately.
-- First_week price increase for all golden cross across all symbols.
-- Compiling them into a single table 
--Query_3 start
create table price_change_over_time as
with w_1 as
(with w_11 as
...
-- 2nd_week price increase for all golden cross across all symbols.
w_2 as
(with w_21 as
(select a.symbol,((s.close-a.close)/a.close)*100 as percent_increase,
case 
...
-- 3rd_week price increase for all golden cross across all symbols.
w_3 as
(with w_31 as
(select a.symbol,((s.close-a.close)/a.close)*100 as percent_increase,
case 
...
-- 1st month price increase for all golden cross across all symbols.
mon_1 as
(with m_11 as
(select a.symbol,((s.close-a.close)/a.close)*100 as percent_increase,
case 
...
-- 2nd month price increase for all golden cross across all symbols.
mon_2 as
(with m_21 as
(select a.symbol,((s.close-a.close)/a.close)*100 as percent_increase,
case 
...
-- 3rd month price increase for all golden cross across all symbols.
mon_3 as
(with m_31 as
(select a.symbol,((s.close-a.close)/a.close)*100 as percent_increase,
case 
...
-- 4th month price increase for all golden cross across all symbols.
mon_4 as
(with m_41 as
(select a.symbol,((s.close-a.close)/a.close)*100 as percent_increase,
case 
...
-- 5th month price increase for all golden cross across all symbols.
mon_5 as
(with m_51 as
(select a.symbol,((s.close-a.close)/a.close)*100 as percent_increase,
case 
...
-- 6th month price increase for all golden cross across all symbols.
mon_6 as
(with m_61 as
(select a.symbol,((s.close-a.close)/a.close)*100 as percent_increase,
case 
...
--Query_3 end
 ...
 ----  Calculate the  percentage of golden cross result in a % price increase for 1st week
 
 --Query_4 start
create table week_1_price_change as 
with test_2 as
(with test_1 as
(select a.symbol,((s.close-a.close)/a.close)*100 as percent_increase,
case 
...
order by symbol;
 ...
 ----  Calculate the  percentage of golden cross result in a % price increase for 2nd week
 --Query_5 start
 create table week_2_price_change as
with test_2 as
(with test_1 as
(select a.symbol,((s.close-a.close)/a.close)*100 as percent_increase,
case 
...
order by symbol;
 ...
 ----  Calculate the  percentage of golden cross result in a % price increase for 3rd week
 --Query_6 start
 create table week_3_price_change as
with test_2 as
(with test_1 as
(select a.symbol,((s.close-a.close)/a.close)*100 as percent_increase,
case 
...
--Query_6 end
 ...
 ----  Calculate the  percentage of golden cross result in a % price increase for 1st month
--Query_7 start 
create table month_1_price_change as
with test_2 as
(with test_1 as
(select a.symbol,((s.close-a.close)/a.close)*100 as percent_increase,
case 
...
--Query_7 end
 ...
 ----  Calculate the  percentage of golden cross result in a % price increase for 2nd month
--Query_8 start
create table month_2_price_change as
with test_2 as
(with test_1 as
(select a.symbol,((s.close-a.close)/a.close)*100 as percent_increase,
case 
...
----  Calculate the  percentage of golden cross result in a % price increase for 3rd month
 
 --Query_9 start
create table month_3_price_change as
with test_2 as
(with test_1 as
(select a.symbol,((s.close-a.close)/a.close)*100 as percent_increase,
case 
...
----  Calculate the  percentage of golden cross result in a % price increase for 4th month
--Query_10 start 
create table month_4_price_change as
with test_2 as
(with test_1 as
(select a.symbol,((s.close-a.close)/a.close)*100 as percent_increase,
case 
...
----  Calculate the  percentage of golden cross result in a % price increase for 5th month
 --Query_11 start
create table month_5_price_change as
with test_2 as
(with test_1 as
(select a.symbol,((s.close-a.close)/a.close)*100 as percent_increase,
case 
...
----  Calculate the  percentage of golden cross result in a % price increase for 6th month
 --Query_12 start
create table month_6_price_change as
with test_2 as
(with test_1 as
(select a.symbol,((s.close-a.close)/a.close)*100 as percent_increase,
case 
